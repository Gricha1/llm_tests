...
def compute_val_metrics(metrics: Dict[str, Any]) -> Dict[str, Any]:
    test_res = []
    for key,val in metrics.items():
        if key.startswith('val-core/') and 'reward/mean' in key:
            test_res.append(float(val))
    avg = np.mean(test_res)
    return {
        'val-core/avg': avg,
    }
...
class RayAEntTrainer(RayPPOTrainer):
    ...
    def fit(self):
        """
        The training loop. Built from "verl/trainer/ppo/ray_trainer"
        """
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        batch.meta_info['entropy_coeff'] = self.entropy_coeff # for adaptive entropy coeff
                        # update actor
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.test_freq == 0):
                        ...
                        metrics.update(compute_val_metrics(val_metrics))
