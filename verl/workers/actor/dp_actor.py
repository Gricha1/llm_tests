    ...
    def __init__(self, config, actor_module: nn.Module, actor_optimizer: torch.optim.Optimizer = None):
        ...
        if self.config.entropy_from_logits_with_chunking:   
            if self.config.get('clamp_entropy'):
                ...
                entropy_from_logits = partial(verl_F.clamped_entropy_from_logits_with_chunking, clamp_p=clamp_p)
                print('Using clamped entropy loss with chunking, clamp_p =', clamp_p)
            else:
            ...
            if self.config.get('clamp_entropy'):
                from functools import partial
                clamp_p = self.config.get('clamp_p')
                entropy_from_logits = partial(verl_F.clamped_entropy_from_logits, clamp_p=clamp_p)
                print('Using clamped entropy loss with clamp_p =',clamp_p)
            else:
